...
const registerUser = asyncHandler(async (req, res) => {
  const { name, email, password, phoneNumber, role,username } = req.body;

  if (
    [name || email || password || phoneNumber|| username].some(
      (field) => field?.trim() === ""
    )
  ) {
...
const socialLogin = asyncHandler(async (req, res) => {
  ...
  res
    .status(200)
    .json({ success: true, message: "Social login successful", accessToken });
});
